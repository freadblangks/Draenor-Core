            ...
            void OnGameObjectCreate(GameObject* go)
            {
                switch (go->GetEntry())
                {
                    case GO_CRUSADERS_CACHE_10:
                        if (instance->GetSpawnMode() == Difficulty::RAID_DIFFICULTY_10MAN_NORMAL)
                            CrusadersCacheGUID = go->GetGUID();
                        break;
                    case GO_CRUSADERS_CACHE_25:
                        if (instance->GetSpawnMode() == Difficulty::RAID_DIFFICULTY_25MAN_NORMAL)
                            CrusadersCacheGUID = go->GetGUID();
                        break;
                    ...
                    case GO_CRUSADERS_CACHE_25_H:
                        if (instance->GetSpawnMode() == Difficulty::RAID_DIFFICULTY_25MAN_HEROIC)
                            CrusadersCacheGUID = go->GetGUID();
                        break;
                    ...
                    case BOSS_ANUBARAK:
                        switch (state)
                        {
                            case DONE:
                            {
                                ...
                                if (instance->GetSpawnMode() == Difficulty::RAID_DIFFICULTY_10MAN_HEROIC)
                                {
                                    if (TrialCounter >= 50)
                                ...
                                else if (instance->GetSpawnMode() == Difficulty::RAID_DIFFICULTY_25MAN_HEROIC)
                                {
                                    if (TrialCounter >= 50)
